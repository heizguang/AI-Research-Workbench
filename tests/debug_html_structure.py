...
def debug_baidu():
    ...
    for i, container in enumerate(containers[:3], 1):
        ...
        for child in container.cssselect("*"):
            ...
            if class_name and ("abstract" in class_name.lower() or "content" in class_name.lower() or "desc" in class_name.lower()):
...
def debug_sogou():
    ...
    for i, container in enumerate(containers[:3], 1):
        ...
        for child in container.cssselect("*"):
            class_name = child.get("class", "")
            if class_name and ("abstract" in class_name.lower() or "content" in class_name.lower() or "desc" in class_name.lower() or "text" in class_name.lower()):
                text = child.text_content().strip()
                print(f"  {child.tag}.{class_name}: {len(text)} 字符")
                if text:
                    print(f"    内容: {text[:100]}...")
